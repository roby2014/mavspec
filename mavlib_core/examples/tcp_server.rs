...
fn listen<R: Read>(reader: R, whoami: String) -> mavlib_core::errors::Result<()> {
...
fn send_heartbeats<W: Write>(writer: W, whoami: String) -> mavlib_core::errors::Result<()> {
...
fn handle_stream(stream: TcpStream, whoami: String) -> mavlib_core::errors::Result<()> {
    ...
    thread::spawn(move || -> mavlib_core::errors::Result<()> { listen(reader, recv_name) });
    // Send heartbeats
    send_heartbeats(stream, send_name)
}

/// Start server and wait until it binds to address.
fn server(address: String) -> mavlib_core::errors::Result<()> {

...
impl BuildHelper {
    ...
    pub fn builder<T: ?Sized + AsRef<OsStr>>(out_path: &T) -> BuildHelperConf {
    ...
    pub fn generate(&self) -> anyhow::Result<()> {
        ...
        let protocol = if let Some(protocol) = &self.protocol {
            ...
            if let Some(messages) = &self.messages {
                protocol.retain_messages(messages, !self.all_enums);
            }
...
impl BuildHelperConf {
    ...
    pub fn set_sources<T: ?Sized + AsRef<OsStr>>(&mut self, sources: &[&T]) -> &mut Self {
        self.0.sources = Some(sources.iter().map(|&src| PathBuf::from(src)).collect());
